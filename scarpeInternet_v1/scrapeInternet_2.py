...
def main():
    ...
    max_breadth = int(
        input("Enter maximum links to follow per page (default 5): ") or 5
    )
    relevance_threshold = float(
        input("Enter relevance threshold (0.0-1.0, default 0.3): ") or 0.3
    )

    # Domain restriction (optional)
    restrict_domains = (
        input("Restrict to specific domains? (y/n, default n): ").lower() == "y"
    )
    allowed_domains = None
    if restrict_domains:
        domains = input(
